...
#Get the 2017 Values for the Other Training & Professional Dues section
```{r}
ISF_sinking_fund_gen_op_other_training_and_prof_dues_2017 <- df_01_sinking_fund_gen_op[18:19, 1:2]
ISF_radio_comm_other_training_and_prof_dues_2017 <- df_03_radio_comm[22:24, 1:2]
ISF_motor_vehicle_mtn_other_training_and_prof_dues_2017 <- df_04_motor_vehicle_mtn[25:27, 1:2]
ISF_printing_reprod_other_training_and_prof_dues_2017 <- df_05_printing_reprod[22:26, 1:2]
ISF_CCA_income_tax_other_training_and_prof_dues_20177 <- df_08_CCA_income_tax[22:25, 1:2]
```

#Merge all the offices into one dataframe

```{r}
all_other_training_and_prof_dues_2017 <- Reduce(function(r_x,r_y) merge(x=r_x, y=r_y, by=1, all=TRUE),  
...
#Rename first column
```{r}
names(all_other_training_and_prof_dues_2017)[1] <- 'other_training_and_prof_dues_2017'
names(all_other_training_and_prof_dues_2017)[2] <- 'Sinking Fund General Operation'
names(all_other_training_and_prof_dues_2017)[3] <- 'Office of Radio Communication'
names(all_other_training_and_prof_dues_2017)[4] <- 'Motor Vehicle Maintenance'
names(all_other_training_and_prof_dues_2017)[5] <- 'Printing and Reproduction'
names(all_other_training_and_prof_dues_2017)[6] <- 'CCA Income Tax Administration'
all_other_training_and_prof_dues_2017[5,1] <- "Total" #renaming row to "total"

all_other_training_and_prof_dues_2017 <- clean_names(all_other_training_and_prof_dues_2017) #cleans the column names
```

#Create file
```{r}
write.csv(all_other_training_and_prof_dues_2017,"all_other_training_and_prof_dues_2017.csv")
```
