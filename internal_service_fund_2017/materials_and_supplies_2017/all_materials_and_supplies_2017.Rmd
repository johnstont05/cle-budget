---
title: "all_materials_and_supplies_2017"
author: "Taylor"
date: "9/22/2020"
...
```{r}
ISF_sinking_fund_gen_op_materials_and_supplies_2017 <- df_01_sinking_fund_gen_op[25:26, 1:2]
ISF_telephone_exc_materials_and_supplies_2017 <- df_02_telephone_exc[40:45, 1:2]
ISF_radio_comm_materials_and_supplies_2017 <- df_03_radio_comm[39:44, 1:2]
ISF_motor_vehicle_mtn_materials_and_supplies_2017 <- df_04_motor_vehicle_mtn[42:67, 1:2]
ISF_printing_reprod_materials_and_supplies_2017 <- df_05_printing_reprod[41:51, 1:2]
ISF_CCA_income_tax_materials_and_supplies_2017 <- df_08_CCA_income_tax[44:53, 1:2]
```

#Merge all the offices into one dataframe

```{r}
all_materials_and_supplies_2017 <- Reduce(function(r_x,r_y) merge(x=r_x, y=r_y, by=1, all=TRUE),  
...
#Rename first column
```{r}
names(all_materials_and_supplies_2017)[1] <- 'materials_and_supplies_2017'
names(all_materials_and_supplies_2017)[2] <- 'Sinking Fund General Operation'
names(all_materials_and_supplies_2017)[3] <- 'Telephone Exchange'
names(all_materials_and_supplies_2017)[4] <- 'Office of Radio Communication'
names(all_materials_and_supplies_2017)[5] <- 'Motor Vehicle Maintenance'
names(all_materials_and_supplies_2017)[6] <- 'Printing and Reproduction'
names(all_materials_and_supplies_2017)[7] <- 'CCA Income Tax Administration'
all_materials_and_supplies_2017[34,1] <- "Total" #renaming row to "total"

all_materials_and_supplies_2017 <- clean_names(all_materials_and_supplies_2017) #cleans the column names
```

#Create file
```{r}
write.csv(all_materials_and_supplies_2017,"all_materials_and_supplies_2017.csv")
```
