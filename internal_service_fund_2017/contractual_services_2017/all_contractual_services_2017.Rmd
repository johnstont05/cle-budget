---
title: "all_contractual_services_2017"
author: "Taylor"
date: "9/22/2020"
...
```{r}
ISF_sinking_fund_gen_op_other_training_and_prof_contractual_services_2017 <- df_01_sinking_fund_gen_op[21:23, 1:2]
ISF_telephone_exc_contractual_services_2017 <- df_02_telephone_exc[35:38, 1:2]
ISF_radio_comm_contractual_services_2017 <- df_03_radio_comm[33:37, 1:2]
ISF_motor_vehicle_mtn_contractual_services_2017 <- df_04_motor_vehicle_mtn[36:40, 1:2]
ISF_printing_reprod_contractual_services_2017 <- df_05_printing_reprod[34:39, 1:2]
ISF_health_insurance_fund_contractual_services_2017 <- df_06_health_insurance_fund[1:11, 1:2]
ISF_CCA_income_tax_contractual_services_2017 <- df_08_CCA_income_tax[31:42, 1:2]
```

#Merge all the offices into one dataframe

```{r}
all_contractual_services_2017 <- Reduce(function(r_x,r_y) merge(x=r_x, y=r_y, by=1, all=TRUE),  
...
#Rename first column
```{r}
names(all_contractual_services_2017)[1] <- 'contractual_services_2017'
names(all_contractual_services_2017)[2] <- 'Sinking Fund General Operation'
names(all_contractual_services_2017)[3] <- 'Telephone Exchange'
names(all_contractual_services_2017)[4] <- 'Office of Radio Communication'
names(all_contractual_services_2017)[5] <- 'Motor Vehicle Maintenance'
names(all_contractual_services_2017)[6] <- 'Printing and Reproduction'
names(all_contractual_services_2017)[7] <- 'Health Insurance Fund'
names(all_contractual_services_2017)[8] <- 'CCA Income Tax Administration'
all_contractual_services_2017[27,1] <- "Total" #renaming row to "total"

all_contractual_services_2017 <- clean_names(all_contractual_services_2017) #cleans the column names
```

#Create file
```{r}
write.csv(all_contractual_services_2017,"all_contractual_services_2017.csv")
```
