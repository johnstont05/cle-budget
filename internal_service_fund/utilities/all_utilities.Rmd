...
```{r}
ISF_telephone_exc_utilities_2017 <- df_02_telephone_exc[26:33, 1:2]
ISF_radio_comm_utilities_2017 <- df_03_radio_comm[26:31, 1:2]
ISF_motor_vehicle_mtn_utilities_2017 <- df_04_motor_vehicle_mtn[29:34, 1:2]
ISF_printing_reprod_utilities_2017 <- df_05_printing_reprod[28:32, 1:2]
ISF_CCA_income_tax_utilities_2017 <- df_08_CCA_income_tax[27:29, 1:2]
```

#Merge all the offices into one dataframe

```{r}
all_utilities_2017 <- Reduce(function(r_x,r_y) merge(x=r_x, y=r_y, by=1, all=TRUE),  
...
#Rename first column
```{r}
names(all_utilities_2017)[1] <- 'utilities_2017'
names(all_utilities_2017)[2] <- 'Telephone Exchange'
names(all_utilities_2017)[3] <- 'Office of Radio Communication'
names(all_utilities_2017)[4] <- 'Motor Vehicle Maintenance'
names(all_utilities_2017)[5] <- 'Printing and Reproduction'
names(all_utilities_2017)[6] <- 'CCA Income Tax Administration'
all_utilities_2017[13,1] <- "Total" #renaming row to "total"

all_utilities_2017 <- clean_names(all_utilities_2017) #cleans the column names

```

#Create file
```{r}
write.csv(all_utilities_2017,"all_utilities_2017.csv")
```
