...
#Get the 2017 Values for the Salary and Wages section
```{r}
ISF_sinking_fund_gen_op_salary_2017 <- df_01_sinking_fund_gen_op[2:6, 1:2]
ISF_telephone_exc_salary_2017 <- df_02_telephone_exc[2:8, 1:2]
ISF_radio_comm_salary_2017 <- df_03_radio_comm[2:7, 1:2]
ISF_motor_vehicle_mtn_salary_2017 <- df_04_motor_vehicle_mtn[2:9, 1:2]
ISF_printing_reprod_salary_2017 <- df_05_printing_reprod[2:8, 1:2]
ISF_CCA_income_tax_salary_2017 <- df_08_CCA_income_tax[2:9, 1:2]
```

#Merge all the offices into one dataframe

```{r}
all_salary_and_wages_2017 <- Reduce(function(r_x,r_y) merge(x=r_x, y=r_y, by=1, all=TRUE),  
...
#Rename first column
```{r}
names(all_salary_and_wages_2017)[1] <- 'salary_and_wages_2017'
names(all_salary_and_wages_2017)[2] <- 'Sinking Fund General Operation'
names(all_salary_and_wages_2017)[3] <- 'Telephone Exchange'
names(all_salary_and_wages_2017)[4] <- 'Office of Radio Communication'
names(all_salary_and_wages_2017)[5] <- 'Motor Vehicle Maintenance'
names(all_salary_and_wages_2017)[6] <- 'Printing and Reproduction'
names(all_salary_and_wages_2017)[7] <- 'CCA Income Tax Administration'
all_salary_and_wages_2017[9,1] <- "Total" #renaming row to "total"

all_salary_and_wages_2017 <- clean_names(all_salary_and_wages_2017) #cleans the column names
```

#Create file
```{r}
write.csv(all_salary_and_wages_2017,"all_salary_and_wages_2017.csv")
```
